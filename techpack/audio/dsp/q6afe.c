...
#ifdef DEBUG
		pr_info("%s: sending, %d bytes to be write\n", __func__, 
				cmd_size);
...
#ifdef DEBUG
		pr_info("%s: get result done %d, %d bytes responded\n", __func__, result,
				get_resp_v2->param_size);
		for (i = 0; i < get_resp_v2->param_size; ++i)
			pr_info("%i: %02x\n", i, buf8[i]);
#endif
...
#ifdef DEBUG
		pr_info("%s: get result done %d, %d bytes responded\n", __func__, result,
				get_resp_v3->param_size);
		for (i = 0; i < get_resp_v3->param_size; ++i)
			pr_info("%i: %02x\n", i, buf8[i]);
#endif
	}
err:
...
{
	const int32_t port_id = AFE_PORT_ID_QUATERNARY_MI2S_RX;
	union afe_spkr_prot_config prot_config;
