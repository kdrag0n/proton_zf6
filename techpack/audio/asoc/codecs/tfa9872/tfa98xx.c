		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("TFA%d control %d", tfa98xx->tfa->dev_idx, ucontrol->value.integer.value[0]);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = 
		(tfa98xx->flags & TFA98XX_FLAG_CHIP_SELECTED)?1:0;
	mutex_unlock(&tfa98xx_mutex);

	return 0;
}

static int tfa98xx_create_selector_control(struct tfa98xx *tfa98xx)
{
	const char *ctrl_name;
	char *name;
	struct device_node *np;
	struct snd_kcontrol_new *tfa98xx_controls;
	int ret;

	np = tfa98xx->i2c->dev.of_node;

	if (!np) {
		pr_warn("ofnode not found\n");
		return 0;
	}

	ret = of_property_read_string(np, 
			"asus,control-name", &ctrl_name);
	if (ret) {
		pr_warn("asus,control-name not found %d!\n", ret);
		return 0;
	}
	...
	tfa98xx_controls = devm_kzalloc(tfa98xx->codec->dev,
			sizeof(tfa98xx_controls[0]), GFP_KERNEL);

	if (!tfa98xx_controls)
		return -ENOMEM;

	scnprintf(name, MAX_CONTROL_NAME, "TFA_Selector %s", ctrl_name);
	tfa98xx_controls[0].name = name;
	tfa98xx_controls[0].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[0].info = tfa98xx_info_stereo_ctl;
	tfa98xx_controls[0].get = tfa98xx_get_stereo_ctl;
	tfa98xx_controls[0].put = tfa98xx_set_stereo_ctl;
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	pr_info("tfa98xx_create_controls");
...
#ifdef TFA9874_NONDSP_STEREO
	tfa98xx_create_selector_control(tfa98xx);
#endif
...
#ifdef TFA9874_NONDSP_STEREO
	if (!(tfa98xx->flags & TFA98XX_FLAG_CHIP_SELECTED)) {
		pr_debug("%d: init not selected.", tfa98xx->tfa->dev_idx);
		return ;
	}
#endif
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	init_completion(&tfa98xx->fw_complete);
	...
	wait_for_completion_timeout(&tfa98xx->fw_complete, msecs_to_jiffies(5000));
	// This prevents any further audio actions before firmware is done
