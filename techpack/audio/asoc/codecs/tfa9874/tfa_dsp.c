...
/* the ops wrapper for tfa98_toggle_mtp_clock */
enum Tfa98xx_Error tfa98xx_set_mtp_clk(struct tfa_device *tfa, int state)
{
	return (tfa->dev_ops.set_mtp_clk)(tfa, state);
}
...
/* the wrapper to store mtp, in case of full */
enum Tfa98xx_Error tfa98xx_mtp_storage(struct tfa_device *tfa, int stored, int calibrated)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	error = (tfa->dev_ops.mtp_storage)(tfa, stored);

	return error;
}

/* the wrapper for DspReset, in case of full */
enum Tfa98xx_Error tfa98xx_load_mtp_setting(struct tfa_device *tfa, int stored, int calibrated)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	error = (tfa->dev_ops.load_mtp_settings)(tfa, stored);
...
enum Tfa98xx_Error tfa98xx_set_mtp(struct tfa_device *tfa, uint16_t value, uint16_t mask)
{
	...
	/* Assure there is a clock to MTP (enable it when neccesery) */
	error = tfa98xx_set_mtp_clk(tfa, 1);
	if (error) {
		return error;
	}
	if (tfa->verbose) {
	...
	/* Disable the MTP, if this is neccessary */
	error = tfa98xx_set_mtp_clk(tfa, 0);
	if (error) {
		return error;
	}
	if (tfa->verbose) {
...
enum Tfa98xx_Error dsp_msg(struct tfa_device *tfa, int length24, const char *buf24)
{
	...
	if (tfa->convert_dsp32) {
		...
		length = 4 * length24 / 3;
		intbuf = kmalloc(length, GFP_KERNEL);
		buf = (char *)intbuf;
...
enum Tfa98xx_Error tfaRunStartup(struct tfa_device *tfa, int profile)
{
	...
	err = (enum Tfa98xx_Error)(tfa_dev_set_state(tfa, TFA_STATE_INIT_CF));
	if (err)
	    return err;
...
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance(struct tfa_device *tfa)
{
	...
	int nr_bytes, i, data[2], calibrateDone, spkr_count=0;
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC)) {
		...
		if (!tfa->is_probus_device) {
			for (i = 0; i < 2; i++) {
				...
				if (!calibrateDone) {
					if (i > 0) {
						pr_debug("CalibrateDone is not set!\n");
						return Tfa98xx_Error_Fail;
					}
					else {
		...
		if (error == Tfa98xx_Error_Ok) {
			...
			for(i=0; i<spkr_count; i++) {
				...
				if(tfa->tfa_family == 2)
					tfa->mohm[i] = (scaled_data*1000)/TFA2_FW_ReZ_SCALE;
				else
					tfa->mohm[i] = (scaled_data*1000)/TFA1_FW_ReZ_SCALE;
			}
		}
	}
...
enum Tfa98xx_Error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_INIT_CF:      /* coolflux HW access possible (~initcf) */
		...
		/* Enable MTP when clock is stable, to avoid MTP corruption */
		err = tfa98xx_set_mtp_clk(tfa, 1);
		if (tfa->verbose) {
			pr_debug("MTP clock enabled (err:%d).\n", err);
		}
#ifdef MTP_HOST_FUNCTION
        if (tfa->dev_ops.mtp_storage)
            err = tfa->dev_ops.mtp_storage( tfa,
                                            tfa->stored);

        if (tfa->dev_ops.load_mtp_settings)
            err = tfa->dev_ops.load_mtp_settings(tfa, tfa->stored);
#endif
		break;
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		*/
		...
		err = tfa98xx_set_mtp_clk(tfa, 0);
		if (tfa->verbose) {
			pr_debug("MTP clock disabled (err:%d).\n", err);
		}
