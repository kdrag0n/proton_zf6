...
struct tfa_device_ops {
	...
	enum Tfa98xx_Error (*set_mtp_clk)(struct tfa_device *tfa, int state); /**< Allow to change MTP clock setting */
	enum Tfa98xx_Error(*set_osc_powerdown)(struct tfa_device *tfa, int state); /**< Allow to change internal osc. gating settings */
    enum Tfa98xx_Error (*mtp_storage)(struct tfa_device *tfa, int stored); /**< store the original MTP settings */
    enum Tfa98xx_Error (*load_mtp_settings)(struct tfa_device *tfa, int stored); /**< load the calibrated mtp settings to shadow registers */
};
...
struct tfa_device {
    ...
    int stored; /**< flag to indicated if mtp settings is stored*/
    uint16_t mtpsettings[16]; /**< mtp settings which needs to be saved to the host */
};
