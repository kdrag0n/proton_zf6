...
/* GLOBAL (Defaults)                                                               */
/***********************************************************************************/
#ifdef MTP_HOST_FUNCTION
/*
* Tfa98xx_mtp_storage will store the mtp settings to tfa device structure
*/
static enum Tfa98xx_Error tfa98xx_mtp_storage(struct tfa_device *tfa, int stored)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    unsigned int i = 0;
    unsigned short value = 0;
    unsigned char mtpaddress = 0x81;
    FILE *mtpfile = fopen("storemtp.txt", "r");
    ...
    if (!mtpfile)
    {
        mtpfile = fopen("storemtp.txt", "w+");

        for (i=1;i<(sizeof(tfa->mtpsettings))/sizeof(tfa->mtpsettings[0]);i++)
        {
            error = tfa98xx_read_register16(tfa, mtpaddress, &value);
            tfa->mtpsettings[i] = value;
            fprintf(mtpfile, "%d\n", value);
            mtpaddress++;
        }
        fclose(mtpfile);
...
/*
* Tfa98xx_load_mtp_settings will restore the mtp settings to tfa device structure
*/
static enum Tfa98xx_Error tfa98xx_load_mtp_settings(struct tfa_device *tfa, int stored)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    unsigned int i = 0;
    ...
    if ((stored) || (mtpfile))
    {
	    ...
	    xor = value ^ 0x005A;
	    error =  reg_write(tfa, 0x60, xor);

	    for (i=1;i<(sizeof(tfa->mtpsettings))/sizeof(tfa->mtpsettings[0]);i++)
        {
            fscanf(mtpfile, "%hu", &tfa->mtpsettings[i]);
            error = reg_write(tfa, mtpaddress, tfa->mtpsettings[i]);
            mtpaddress++;
        }
        /* SL: currently not writing the contents to MTP itself
        but only shadow registers*/
        /*error =  reg_write(tfa, 0x62, 0x8800);*/
    }
    fclose(mtpfile);
	return error;
}
...
static enum Tfa98xx_Error tfa_set_mtp_clk(struct tfa_device *tfa, int state)
{
	(void)tfa;
	(void)state;
...
void set_ops_defaults(struct tfa_device_ops *ops)
{
	...
	ops->set_mtp_clk = tfa_set_mtp_clk;
	ops->set_osc_powerdown = tfa_set_osc_powerdown;
    ops->mtp_storage = no_overload_function_available;
    ops->load_mtp_settings = no_overload_function_available;
}

/***********************************************************************************/
/* TFA9912                                                                         */
/***********************************************************************************/
static enum Tfa98xx_Error tfa9912_set_mtp_clk(struct tfa_device *tfa, int status)
{
	enum Tfa98xx_Error ret = Tfa98xx_Error_Fail;

	if (tfa) {
		if (status == 0 || status == 1) {
			ret = -(tfa_set_bf(tfa, TFA9912_BF_SSFAIME, status));
		}
	}
...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x1a13) {
		...
		reg_write(tfa, 0xf9, 0x0102); //POR=0x0000
		/* ----- generated code end   ----- */

		/* PLMA5505: MTP key open makes vulanable for MTP corruption */
		tfa9912_set_mtp_clk(tfa, 0);
	} else {
...
void tfa9912_ops(struct tfa_device_ops *ops)
{
	...
	ops->set_mtp_clk = tfa9912_set_mtp_clk;
...
static enum Tfa98xx_Error tfa9874_specific(struct tfa_device *tfa)
{
	...
	switch (tfa->rev) {
		case 0x0a74: /* Initial revision ID */
			...
			reg_write(tfa, 0xf0, 0x0002); //POR=0x0000
			reg_write(tfa, 0xf8, 0x6000); //POR=0x0000
			/* ----- generated code end   ----- */
			break;
		default:
			pr_info("\nWarning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
...
#ifdef MTP_HOST_FUNCTION
    ops->mtp_storage = tfa98xx_mtp_storage;
    ops->load_mtp_settings = tfa98xx_load_mtp_settings;
#else
    ops->mtp_storage = no_overload_function_available;
    ops->load_mtp_settings = no_overload_function_available;
#endif
