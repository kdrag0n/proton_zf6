...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_INPUT_KEY_EVENT:
		pr_debug("%s GF_IOC_INPUT_KEY_EVENT\n", __func__);
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		if (status == 0) {
			...
			pr_info("%s: Succeed to open device. irq = %d\n",
					__func__, gf_dev->irq);
			if (gf_dev->users == 1) {
		...
		pr_info("%s: No device for minor %d\n", __func__, iminor(inode));
	}
...
#ifdef AP_CONTROL_CLK
	pr_info("%s: Get the clk resource.\n", __func__);
	/* Enable spi clock */
