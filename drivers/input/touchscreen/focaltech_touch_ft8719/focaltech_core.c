...
{
    FTS_FUNC_ENTER();
    printk("[FTS][touch] %s ,HW reset touch\n",__func__);
    //printk("[FTS][touch] reset = %d !!!\n", gpio_get_value(fts_data->pdata->reset_gpio));
    ...
    //printk("[FTS][touch] %s ,HW reset touch done\n",__func__);
    FTS_FUNC_EXIT();
    return 0;
}
...
static int fts_power_source_init(struct fts_ts_data *data)
{
    ...
    if (IS_ERR(data->lab)) {
        ret = PTR_ERR(data->lab);
        FTS_ERROR("ret lab regulator failed,ret=%d", ret);
        goto err_get_lab;
    }
    if (regulator_count_voltages(data->lab) > 0) {
        ret = regulator_set_voltage(data->lab, 4000000, 6000000);
        if (ret) {
            FTS_ERROR("lab regulator set_vtg failed ret=%d", ret);
            goto err_set_vtg_lab;
        }
    }
    ...
    if (IS_ERR(data->ibb)) {
        ret = PTR_ERR(data->ibb);
        FTS_ERROR("ret ibb regulator failed,ret=%d", ret);
        goto err_get_ibb;
    }
    if (regulator_count_voltages(data->ibb) > 0) {
        ret = regulator_set_voltage(data->ibb, 4000000, 6000000);
        if (ret) {
            FTS_ERROR("ibb regulator set_vtg failed ret=%d", ret);
            goto err_set_vtg_ibb;
        }
    }
...
err_get_ibb:
    if (regulator_count_voltages(data->ibb) > 0)
        regulator_set_voltage(data->ibb, 0, 6000000);
err_set_vtg_lab:
    regulator_put(data->lab);
err_get_lab:
    if (regulator_count_voltages(data->lab) > 0)
        regulator_set_voltage(data->lab, 0, 6000000);
err_set_vtg_vcc:
...
static int fts_power_source_ctrl(struct fts_ts_data *data, int enable)
{
    ...
    if (enable) {
        ...
        if (!data->power_disabled) {
            ...
            ret = regulator_disable(data->ibb);
            if (ret) {
...
EXPORT_SYMBOL(fts_power_source_ctrl);

int fts_power_source_ctrl_global(int enable)
{
	int ret = 0;
	...
	if (fts_data)
		...
		pr_err("fts_data is null, aborting settting power source.\n");

	return ret;
}
EXPORT_SYMBOL(fts_power_source_ctrl_global);
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    //printk("[fts][touch]: enter fts_read_touchdata\n");
#if FTS_GESTURE_EN
    if (data->suspended) {
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("IRQF_TRIGGER_FALLING:%x", IRQF_TRIGGER_FALLING);
    ret = request_threaded_irq(ts_data->irq, NULL, fts_ts_interrupt,
                               pdata->irq_gpio_flags | IRQF_ONESHOT,
                               ts_data->client->dev.driver->name, ts_data);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    }else if (is_kirin_panel == 0){
	ret = fts_get_dt_coords(dev, "focaltech,display-coords-ara", pdata);
	if (ret < 0)
        FTS_ERROR("Unable to get display-coords-ara");
    }
...
//#endif
    ...
    fts_data->glove_mode_eable = 0;
    fts_data->cover_mode_eable = 0;
    ...
    ts_data->touch_edev = extcon_dev_allocate(asus_touch);
    ts_data->touch_edev->fnode_name = "touch";
    if (extcon_dev_register(ts_data->touch_edev) < 0)
    {
	printk("[Focal][TOUCH_ERR] %s: failed to register switch_dev \n", __func__);
	goto exit_err_edev_register_fail;
    }
    ...
    ts_data->suspend_resume_wq = create_singlethread_workqueue("focal_suspend_resume_wq");
    ...
    if (!ts_data->write_buffer) {
        FTS_ERROR("failed to alloc memory fo write_buffer!");
    }
...
static int fts_ts_remove(struct i2c_client *client)
{
    ...
    kfree_safe(ts_data->point_buf);
    kfree_safe(ts_data->events);
    kfree_safe(ts_data->write_buffer);
...
static void focal_suspend_work(struct work_struct *work)
{
	...
	if ((fts_data->dclick_mode_eable == 1) | (fts_data->swipeup_mode_eable == 1) | (fts_data->gesture_mode_eable == 1)) {
		...
		if (retval == 0)
		{
			...
			retval = enable_irq_wake(fts_data->irq);
			if (retval)
...
static void focal_resume_work(struct work_struct *work)
{
	...
	if (fts_gesture_resume(fts_data->client) == 0)
	{
		...
		fts_irq_disable();
		//fts_reset_proc(300);
		//printk("%s: focal_resume_work pmode_tp_flag = %d\n", __func__, pmode_tp_flag);
...
#if FTS_POWER_SOURCE_CUST_EN
    fts_power_source_ctrl(fts_data, ENABLE);
    //printk("[FTS][touch] %s: enable power\n", __func__);
#endif
...
{
	if (fts_data)
		return ((fts_data->dclick_mode_eable == 1) | (fts_data->swipeup_mode_eable == 1) | (fts_data->gesture_mode_eable == 1));
	else
		return false;
}
EXPORT_SYMBOL(fts_gesture_check);

void fts_set_pmode(bool pmode)
{
	printk("fts_set_pmode = %d \n", pmode);
}
EXPORT_SYMBOL(fts_set_pmode);
/*
*****************************************************************************
*  Name: fts_ts_suspend
*  Brief:
