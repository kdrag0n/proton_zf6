...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    u8 *writebuf = fts_data->write_buffer;
    int buflen = count;
    int writelen = 0;
    int ret = 0;
    char tmp[25];
...
static ssize_t disable_touch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int tmp = 0;
	tmp = buf[0] - 48;
...
static ssize_t disable_touch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	bool stat = 0;
