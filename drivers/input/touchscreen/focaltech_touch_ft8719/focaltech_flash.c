...
static int fts_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    u32 packet_number;
    u32 packet_len = 0;
    u8 *packet_buf = fwupgrade->file_buffer;
    //u8 packet_buf[FTS_FLASH_PACKET_LENGTH + FTS_CMD_WRITE_LEN] = { 0 };
    u8 ecc_tmp = 0;
    int ecc_in_host = 0;

    printk("[FTS][touch][UPGRADE]fts_pram_write_buf");
...
static int fts_pram_write_remap(struct i2c_client *client)
{
    ...
    printk("[FTS][touch][UPGRADE]fts_pram_write_remap");
...
int fts_pram_write_init(struct i2c_client *client)
{
    ...
    printk("[FTS][touch][UPGRADE]fts_pram_write_init");
...
int fts_fwupg_get_boot_state(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
    ...
    printk("[FTS][touch][UPGRADE]fts_fwupg_get_boot_state");
...
int fts_fwupg_reset_to_boot(struct i2c_client *client)
{
    ...
    printk("[FTS][touch][UPGRADE] fts_fwupg_reset_to_boot");
...
int fts_fwupg_enter_into_boot(struct i2c_client *client)
{
    ...
    printk("[FTS][touch][UPGRADE]fts_fwupg_enter_into_boot");
...
int fts_upgrade_bin(struct i2c_client *client, char *fw_name, bool force)
{
    ...
    printk("[FTS][touch][UPGRADE]fts_upgrade_bin");
    ...
    printk("[FTS][touch][UPGRADE]fts_upgrade_bin : upgrade fw bin success");
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    fwupgrade->file_buffer = (u8 *)kzalloc((FTS_FLASH_PACKET_LENGTH + 6), GFP_KERNEL);
    if (!fwupgrade->file_buffer) {
