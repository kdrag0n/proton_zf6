...
static bool cci_init_done = 0;	//ASUS_BSP Byron ref count for check whether CCI Bus init or not

//ASUS_Byron remove xsdn part, we don't have to control xsdn +++
/*
...
static int stmvl53l1_get_dt_info(struct device *dev, struct tof_ctrl_t *t_ctrl)
{
	...
	if (rc) {
		vl53l1_wanrmsg("Unable to find intr-gpio %d %d", rc, t_ctrl->intr_gpio);
		t_ctrl->intr_gpio = -1;
	}
#if 0
	rc = of_property_read_u32_array(dev->of_node, "xsdn-gpio", &t_ctrl->xsdn_gpio, 1);
	if (rc) {
		vl53l1_wanrmsg("Unable to find xsdn-gpio %d %d", rc, t_ctrl->xsdn_gpio);
		t_ctrl->xsdn_gpio = -1;
	}
...
static int32_t stmvl53l1_platform_probe(struct platform_device *pdev)
{
	...
	if (!cam_cci_get_subdev(1)){
		vl53l1_errmsg("can not get cci subdev\n");
		return -EPROBE_DEFER;
	}
	...
	wakeup_source_init(&vl53l1_data->stmvl53l1_wakeup_source, "stmvl53l1_wakeup_source"); //ASUS_BSP Byron add for wake lock use
	/* setup other stuff */
...
release_gpios:
	...
	wakeup_source_trash(&vl53l1_data->stmvl53l1_wakeup_source); //ASUS_BSP Byron add for wake lock use
unregister_subdev:
...
int stmvl53l1_power_up_cci(void *object)
{
	...
	rc = camera_io_init(&tof_ctrl->io_master_info); //ASUS_BSP Move CCI Init to reset_release function

	if (rc < 0)
		vl53l1_errmsg("cci init failed: rc: %d", rc);

	if(rc == 0){
		__pm_stay_awake(&tof_ctrl->vl53l1_data->stmvl53l1_wakeup_source); //ASUS_BSP Byron get wake lock to avoid suspend
	}
...
int stmvl53l1_power_down_cci(void *cci_object)
{
	...
	if(rc == 0){
		__pm_relax(&tof_ctrl->vl53l1_data->stmvl53l1_wakeup_source); //ASUS_BSP Byron release wake lock
	}
...
//ASUS_BSP Byron add for call cci init and release +++
int stmvl53l1_init_release_cci(int if_init, void *object)
{
	int rc=0;
	struct tof_ctrl_t *tof_ctrl = (struct tof_ctrl_t *)object;
	if (!tof_ctrl) {
	...
	if(if_init == 1 && cci_init_done == 0){
		rc = camera_io_init(&tof_ctrl->io_master_info);
		if(rc == 0){
			cci_init_done = 1;
			vl53l1_info("cci init success cci_init_done set to (%d)\n",cci_init_done);
		}
	}else if(if_init == 0 && cci_init_done == 1){
		rc = camera_io_release(&tof_ctrl->io_master_info);
		if(rc == 0){
			cci_init_done = 0;
			vl53l1_info("cci release success cci_init_done set to (%d)\n",cci_init_done);
		}
	}else{
		vl53l1_errmsg("check value %s request cci_init_done(%d)\n",if_init?"init":"release",cci_init_done);
		rc = -1;
	}
	return rc;
}
